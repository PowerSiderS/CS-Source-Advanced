        ...
        m_sConnectCode( pszConnectCode )
{
        ...
        m_pConnectButton->SetText("#ServerBrowser_JoinGame");
        m_pRefreshButton->SetText("#ServerBrowser_Refresh");
        m_pCloseButton->SetText("#ServerBrowser_Close");
...
void CDialogGameInfo::RequestInfo()
{
        ...
        // Always update the labels to "Refreshing..." or clear old info
        SetControlString("ServerText", "Refreshing...");
        SetControlString("GameText", "");
        SetControlString("MapText", "");
        SetControlString("PlayersText", "");
        SetControlString("PingText", "");

        if ( m_iRequestRetry == 0 || system()->GetTimeMillis() > m_iRequestRetry )
        {
...
void CDialogGameInfo::ServerResponded( gameserveritem_t &server )
{
        ...
        m_hPingQuery = HSERVERQUERY_INVALID;
        m_Server = server;
        m_Server.m_bHadSuccessfulResponse = true; // Explicitly set this

        // Update the name if we got a real one
        if ( m_Server.GetName()[0] )
        {
                SetTitle( m_Server.GetName(), true );
        }

        // Make sure we update our labels with the new data
        char buf[128];
        SetControlString( "ServerText", m_Server.GetName() );
        SetControlString( "GameText", m_Server.m_szGameDescription );
        SetControlString( "MapText", m_Server.m_szMap );
...
void CDialogGameInfo::ConnectToServer()
{
        ...
        if ( m_Server.m_bPassword && !m_szPassword[0] )
        {
                ...
                // Ensure the password entry has focus and the dialog is on top
                box->MoveToFront();
                return;
        }
