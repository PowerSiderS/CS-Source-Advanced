...
#define D(a) LauncherArgv[iLastArgs++] = (char*)a
        ...
        // Pass Android hardware info to engine via environment variables for SteamID generation
        char androidId[PROP_VALUE_MAX] = {0};
        char model[PROP_VALUE_MAX] = {0};
        char manufacturer[PROP_VALUE_MAX] = {0};

        // Use dlopen/dlsym to avoid direct dependency on __system_property_get if it's not in the linker path
        typedef int (*pfn_system_property_get)(const char*, char*);
        void* handle = dlopen("libc.so", RTLD_LAZY);
        if (handle)
        {
                pfn_system_property_get get_prop = (pfn_system_property_get)dlsym(handle, "__system_property_get");
                if (get_prop)
                {
        ...
        if (androidId[0]) setenv("ANDROID_ID", androidId, 1);
        if (model[0]) setenv("DEVICE_MODEL", model, 1);
        if (manufacturer[0]) setenv("DEVICE_MANUFACTURER", manufacturer, 1);
