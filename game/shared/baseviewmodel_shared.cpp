...
ConVar viewmodel_recoil( "viewmodel_recoil", "1.0", FCVAR_ARCHIVE, "Viewmodel recoil scale (0=none, 1=normal, 2=exaggerated)", true, 0.0f, true, 2.0f );
...
#if defined( CLIENT_DLL )
        // Apply viewmodel recoil scale
        float flRecoilScale = viewmodel_recoil.GetFloat();
#else
        float flRecoilScale = 1.0f;
#endif

        //FIXME: These are the old settings that caused too many exposed polys on some models
        VectorMA( origin, -pitch * 0.035f * flRecoilScale,      forward,        origin );
        VectorMA( origin, -pitch * 0.03f * flRecoilScale,               right,  origin );
        VectorMA( origin, -pitch * 0.02f * flRecoilScale,               up,             origin);
}
