...
private:
        ...
        // Per-weapon iron-sight scope overlays (AUG, SSG552)
        int m_iAugScopeTexture;
        int m_iSSG552ScopeTexture;
...
void CHudScope::Init( void )
{
        ...
        m_iAugScopeTexture = vgui::surface()->CreateNewTextureID();
        vgui::surface()->DrawSetTextureFile(m_iAugScopeTexture, "vgui/hud/aug_scope", true, false);

        m_iSSG552ScopeTexture = vgui::surface()->CreateNewTextureID();
        vgui::surface()->DrawSetTextureFile(m_iSSG552ScopeTexture, "vgui/hud/sg552_scope", true, false);
}
...
void CHudScope::Paint( void )
{
        ...
        // Draw weapon-specific iron-sight scope overlay for AUG and SSG552
        ...
        if ( weaponID == WEAPON_AUG || weaponID == WEAPON_SG552 )
        {
                if ( pPlayer->GetFOV() < pPlayer->GetDefaultFOV() )
                {
                        ...
                        int texID = ( weaponID == WEAPON_AUG ) ? m_iAugScopeTexture : m_iSSG552ScopeTexture;

                        float uv1 = 0.5f / 256.0f, uv2 = 1.0f - uv1;
                        vgui::Vertex_t vert[4];
                        vert[0].Init( Vector2D( 0,          0          ), Vector2D( uv1, uv1 ) );
                        vert[1].Init( Vector2D( screenWide, 0          ), Vector2D( uv2, uv1 ) );
                        vert[2].Init( Vector2D( screenWide, screenTall ), Vector2D( uv2, uv2 ) );
                        vert[3].Init( Vector2D( 0,          screenTall ), Vector2D( uv1, uv2 ) );
