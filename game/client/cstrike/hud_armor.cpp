...
{
        // ============================================
        // FIXED: استخدام نفس اللون للـ ProgressBar
        // ============================================
        ...
        if ( pPlayer )
        {
                ...
                {
                        // FIXED: رفع الـ ProgressBar قليلاً للأعلى
                        float adjustedYPos = progress_ypos - YRES(2); // يمكن تغيير القيمة من 2 إلى 3 أو 4 حسب الحاجة
                        
                        // draw the progress bar background
                        DrawBox( progress_xpos, adjustedYPos, progress_wide, progress_tall, m_ProgressBgColor, 1.0f );
                        
                        // FIXED: رسم الـ foreground بنفس لون الـ Armor
                        if ( iArmorValue > 0 )
                        {
                                // استخدام hudColor بدلاً من m_ProgressFgColor
                                DrawBox( progress_xpos, adjustedYPos, progress_wide * Clamp( iArmorValue / 100.0f, 0.0f, 1.0f ), progress_tall, hudColor, 1.0f );
                        }
                }
