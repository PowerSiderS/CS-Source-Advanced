...
static ConVar cl_killfeed_csgo( "cl_killfeed_csgo", "0", FCVAR_ARCHIVE );
...
void CHudDeathNotice::Paint()
{
        ...
        for ( int i = 0; i < iCount; i++ )
        {
                ...
                int y = yStart + ((m_flLineHeight + XRES(2)) * i); // -3 pixels roughly from XRES(5) to XRES(2)
                ...
                int iTotalWide = victimNameLen + victimClanLen + iconWide;
                if ( m_DeathNotices[i].bHeadshot ) iTotalWide += iconHeadshotWide;
                if ( !m_DeathNotices[i].iSuicide ) iTotalWide += UTIL_ComputeStringWidth( m_hTextFont, killer ) + UTIL_ComputeStringWidth( m_hTextFont, killerclan );
                if ( m_DeathNotices[i].bAssisted ) iTotalWide += UTIL_ComputeStringWidth( m_hTextFont, assistplussign ) + UTIL_ComputeStringWidth( m_hTextFont, assister ) + UTIL_ComputeStringWidth( m_hTextFont, assisterclan );
                if ( m_DeathNotices[i].iDominationImageId >= 0 ) iTotalWide += dominationDrawWidth;

                int xMargin = XRES(2);
                iTotalWide += xMargin * 2;
                ...
                // Add extra margin if there are domination/nemesis icons
                if (m_DeathNotices[i].iDominationImageId >= 0)
                {
                    iTotalWide += XRES(1);
                }

                if ( cl_killfeed_csgo.GetBool() ) iTotalWide += XRES(12);
                ...
                if ( cl_killfeed_csgo.GetBool() )
                {
                        float flAlpha = 1.0f;
                        float flLifeRemaining = m_DeathNotices[i].flDisplayTime - gpGlobals->curtime;
                        if ( flLifeRemaining < 0.5f ) flAlpha = flLifeRemaining / 0.5f;

                        Color bgColor( 0, 0, 0, (int)(188.0f * flAlpha) );
                        int iLocalPlayerIndex = engine->GetLocalPlayer();
                        
                        // Highlight logic for CSGO style
                        if ( iLocalPlayerIndex > 0 )
                        {
                                bool bIsLocalVictim = ( iLocalPlayerIndex == m_DeathNotices[i].Victim.iEntIndex );
                                bool bIsLocalKiller = ( iLocalPlayerIndex == m_DeathNotices[i].Killer.iEntIndex );
                                bool bIsLocalAssister = ( iLocalPlayerIndex == m_DeathNotices[i].Assister.iEntIndex );

                                        if ( bIsLocalVictim )
                                        {
                                                // I was killed
                                                bgColor.SetColor( 255, 0, 0, (int)(188.0f * flAlpha) );
                                                surface()->DrawSetColor( Color( 255, 0, 0, (int)(255.0f * flAlpha) ) );
                                                surface()->DrawOutlinedRect( x - 1, y - 2, x + iTotalWide + 1, y + m_flLineHeight - 1 );
                                                surface()->DrawOutlinedRect( x - 2, y - 3, x + iTotalWide + 2, y + m_flLineHeight );
                                        }
                                        else if ( bIsLocalKiller || bIsLocalAssister )
                                        {
                                                // I killed someone or assisted
                                                surface()->DrawSetColor( Color( 255, 0, 0, (int)(255.0f * flAlpha) ) );
                                                surface()->DrawOutlinedRect( x - 1, y - 2, x + iTotalWide + 1, y + m_flLineHeight - 1 );
                                                surface()->DrawOutlinedRect( x - 2, y - 3, x + iTotalWide + 2, y + m_flLineHeight );
                                        }
                                ...
                                surface()->DrawFilledRect( x, y - 2, x + iTotalWide, y + m_flLineHeight - 1 );
                        }
                ...
                if (m_DeathNotices[i].iDominationImageId >= 0)
                {
                        float flAlphaIcon = 1.0f;
                        float flLifeRemainingIcon = m_DeathNotices[i].flDisplayTime - gpGlobals->curtime;
                        if ( flLifeRemainingIcon < 0.5f ) flAlphaIcon = flLifeRemainingIcon / 0.5f;

                        CHudTexture *pIcon = NULL;
                        if ( m_DeathNotices[i].iDominationImageId == m_iDominatedImageId )
                                pIcon = m_iconD_dominated;
                        else if ( m_DeathNotices[i].iDominationImageId == m_iRevengeImageId )
                                pIcon = m_iconD_revenge;

                        if ( pIcon )
                        {
                                pIcon->DrawSelf( x, y + 1, dominationDrawWidth, dominationDrawHeight, Color(255, 255, 255, (int)(255.0f * flAlphaIcon)) );
                        }
                        else
                        {
                                surface()->DrawSetTexture(m_DeathNotices[i].iDominationImageId);
                                surface()->DrawSetColor(Color(255, 255, 255, (int)(255.0f * flAlphaIcon)));
                                surface()->DrawTexturedRect( x, y + 1, x + dominationDrawWidth, y + dominationDrawHeight + 1 );
                        }
                        x += dominationDrawWidth + XRES(1);
                }
                ...
                if ( !m_DeathNotices[i].iSuicide )
                {
                        ...
                        y -= 1; // Correction for DrawGetTextPos side effect
                        ...
                        if ( cl_killfeed_csgo.GetBool() ) x += XRES(3);
                        ...
                        if ( m_DeathNotices[i].bAssisted )
                        {
                                ...
                                x += XRES(2); // 2+ pixels after victim name if assist found (applied to assister name end)
                                if ( cl_killfeed_csgo.GetBool() ) x += XRES(2);
                        }
                }
                ...
                Color iconColor( 255, 255, 255, 255 ); // Default to white for weapons in CSGO mode
                if ( cl_killfeed_csgo.GetBool() )
                {
                        float flAlpha = 1.0f;
                        float flLifeRemaining = m_DeathNotices[i].flDisplayTime - gpGlobals->curtime;
                        if ( flLifeRemaining < 0.5f ) flAlpha = flLifeRemaining / 0.5f;
                        iconColor.SetColor( 255, 255, 255, (int)(255.0f * flAlpha) );
                }
                else
                {
                        iconColor.SetColor( 255, 80, 0, 255 ); // Default orange-red for non-CSGO
                }
                
                int iDeathIconX = x;
                icon->DrawSelf( x, y + 1, iconWide, iconTall, iconColor );
                x += iconWide + XRES(1); // 1 pixel between weapon icon/headshot and name victim
                if ( cl_killfeed_csgo.GetBool() ) x += XRES(1);

                if ( m_DeathNotices[i].bHeadshot )
                {
                        m_iconD_headshot->DrawSelf( x, y + 1, iconHeadshotWide, iconHeadshotTall, iconColor );
                        x += iconHeadshotWide + XRES(1); // 1 pixel between weapon icon/headshot and name victim
                        if ( cl_killfeed_csgo.GetBool() ) x += XRES(1);
                }

                if ( cl_killfeed_csgo.GetBool() )
                {
                        // Additional logic for revenge/dominating icons if they were drawn
                        if (m_DeathNotices[i].iDominationImageId >= 0)
                        {
                                // Spacing already added by DrawTexturedRect logic if needed
                        }
                        
                        // Headshot icon color is handled above, but ensuring other possible overlays match
                        // such as domination/revenge icons which are drawn before names
                }

                // Draw victims clan
                surface()->DrawSetTextColor( m_DeathNotices[i].Victim.color );
                surface()->DrawSetTextPos( x + XRES(2), y + 1 );
