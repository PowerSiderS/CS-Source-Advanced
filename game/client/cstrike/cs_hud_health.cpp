...
void CHudHealth::Paint( void )
{
        ...
        if ( hud_healtharmor_style.GetInt() == HUD_STYLE_DEFAULT )
        {
                ...
                // FIXED: رسم الـ foreground بنفس لون الـ Health
                if ( m_iHealth > 0 )
                {
                        // استخدام hudColor بدلاً من m_ProgressFgColor
                        DrawBox( progress_xpos, adjustedYPos, progress_wide * Clamp( m_iHealth / 100.0f, 0.0f, 1.0f ), progress_tall, hudColor, 1.0f );
                }
        }
