...
static ConVar cl_rainbow_flashlight( "r_rainbow_flashlight", "0", FCVAR_ARCHIVE, "Enable rainbow flashlight effect." );
...
void CFlashlightEffect::UpdateLightNew(const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp )
{
        ...
        extern ConVar cl_rainbow_flashlight;
        if ( cl_rainbow_flashlight.GetBool() )
        {
                float flHue = fmod( gpGlobals->curtime * 100.0f, 360.0f );
                float flSaturation = 1.0f;
                float flValue = 1.0f;

                float c = flValue * flSaturation;
                float x = c * ( 1.0f - fabs( fmod( flHue / 60.0f, 2.0f ) - 1.0f ) );
                float m = flValue - c;

                float r, g, b;
                if ( flHue < 60.0f ) { r = c; g = x; b = 0; }
                else if ( flHue < 120.0f ) { r = x; g = c; b = 0; }
                else if ( flHue < 180.0f ) { r = 0; g = c; b = x; }
                else if ( flHue < 240.0f ) { r = 0; g = x; b = c; }
                else if ( flHue < 300.0f ) { r = x; g = 0; b = c; }
                else { r = c; g = 0; b = x; }
